...
The third quartile is lower than the mean income.
...
```{r}
boot_strap_sample <- rep_sample_n(train,size = 115, reps = 1000, replace = TRUE) %>% 
  group_by(replicate) %>% 
...
```{r}
  ...
  rep_sample_n(train, size = 115, reps = 1000, replace = TRUE) %>% 
